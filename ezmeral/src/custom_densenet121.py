...
def _load_state_dict(model, model_url, model_dir, progress):
    ...
    # to find such keys.
    pattern = re.compile(r'^(.*denselayer\d+\.(?:norm|ELU|conv))\.((?:[12])\.(?:weight|bias|running_mean|running_var))$')
...
def _densenet(arch, growth_rate, block_config, num_init_features, pretrained, progress, **kwargs):
    model_dir = kwargs.pop('online_pretrained_weight_store_path', None)
...
class Custom_Densenet121():
    ...
    def __init__(self, number_classes=5, activations='relu', last_activation=None, pretrained=True, online_pretrained_weight_store_path=None):
        ...
        self.online_pretrained_weight_store_path = online_pretrained_weight_store_path
        
        # model
        self.model = DenseNet121(pretrained=self.pretrained , last_activation=self.last_activation, activations=self.activations, num_classes=self.number_classes, online_pretrained_weight_store_path=self.online_pretrained_weight_store_path)
