...
class Custom_MultiLabel_AlphaBalanced_FocalLoss(torch.nn.Module): 
    ...
    """
    def __init__(self, alpha=None, gamma=2, num_classes=5, device_type='cuda'):
        ...
        if alpha is None:
            ...
            self.alpha = self.alpla.to(device_type)
        else:
            self.alpha = torch.tensor(alpha).to(device_type)

        # gamma is parameter for smoothing the hardness of sample. 
        self.gamma = torch.tensor(gamma).to(device_type)
