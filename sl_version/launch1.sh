...
../../scripts/bin/run-swci -ti --rm --name=swci1 --network=host-1-net --usr-dir=swci --init-script-name=swci-init --key=lung-pyt-cert/cert/swci-1-key.pem --cert=lung-pyt-cert/cert/swci-1-cert.pem --capath=lung-pyt-cert/cert/ca/capath -e http_proxy= -e https_proxy= --apls-ip=10.60.3.50 -e SWCI_TASK_MAX_WAIT_TIME=360
